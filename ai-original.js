...
async function getBestModel() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            const bestModel = data.models.find(m => m.name.includes('gemini-flash-latest') && m.supportedGenerationMethods?.includes('generateContent')) ||
                              data.models.find(m => m.name.includes('gemini-3.5-flash') && m.supportedGenerationMethods?.includes('generateContent')) ||
...
const fallbackModels = [dynamicModel, 'gemini-flash-latest', 'gemini-3.5-flash'];
